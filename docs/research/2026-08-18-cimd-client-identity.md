...
**Status:** Research and architecture recommendation; no implementation started
...
| Event | Fields |
|---|---|
| `oauth_cimd_resolved` | `clientIdUrl`, `redirectUriCount`, `cacheHit`, `durationMs` |
| `oauth_cimd_rejected` | `clientIdUrl`, `reason` (the fetch/validation vocabulary above) |
| `oauth_cimd_blocked` | `clientIdUrl`, `reason` ∈ `blocked_address` \| `host_not_allowed` \| `redirect_refused` |

`clientIdUrl` is attacker-controlled and must be documented as untrusted in the same style as the
existing `registeredClientId` and `redirectUri` comments. `oauth_cimd_blocked` is the alertable
event: a hit means someone aimed ARC-1 at an address it refused to reach.
...
| X5 | Each blocked range: IPv4 private/loopback/link-local/CGNAT, IPv6 ULA/link-local, both IPv4-mapped forms | Refused before connect; `oauth_cimd_blocked` emitted |
...
- resolution, rejection, and blocking are observable through the typed audit union and documented as
  untrusted where applicable;
...
**Operability.** Three configuration variables, three audit events, one alertable condition
(`oauth_cimd_blocked`). Operators gain the measurement that decides SEC-15's remaining scope.
